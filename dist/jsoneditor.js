...
// From MDN
  ...
  JSONEditor.defaults.editors.string = JSONEditor.AbstractEditor.extend({
      ...
      refreshValue: function() {
          ...
          // Add listener to an event triggered by AutoComplete.js so that
          // jsoneditor updates the schema based on the selected value.                
          $(self.input).off('autoCompleteItemSelected').on('autoCompleteItemSelected', function() {
              self.value = self.input.value;
              if(typeof self.value !== "string") self.value = '';
      ...
      layoutEditors: function() {
          ...
          // Normal layout
              ...
              $each(this.property_order, function(i,key) {
                  ...
                  if(editor.table) row.classList.add('table-editor');
                  container.appendChild(row);
...
// Enum Editor (used for objects and arrays with enumerated values)
          ...
          $(this.input).autocomplete({
              ...
              select: (event, ui) => {
                  if (ui.item)
                  {
                      ...
                      this.input.value = ui.item.value;
                  }
              }
          });
      }
  ...
  JSONEditor.defaults.editors.radio = JSONEditor.defaults.editors.checkbox.extend({
      register: function() {
          this._super();
          if(!this.input) return;
          /*
          To create name from formname,
          root[health_history][current_medical_history][index][primary_diagnosis] as formname transfers to
          root[health_history][current_medical_history][primary_diagnosis] and set as name to group radio buttons
           */
          this.input.setAttribute('name', this.formname.replace(/\[[0-9]\]/g,''));
      },
      build: function() {
          var self = this;
          if(!this.options.compact) {
          ...
          this.input = this.theme.getRadioButton();
          this.control = this.theme.getFormControl(this.label, this.input, this.description);
          ...
          this.input.addEventListener('change',function(e) {
              ...
              for (let obj of self.parent.parent.value) {
                  obj[self.key] = false;
              }
  ...
  JSONEditor.defaults.iconlibs.alaya = JSONEditor.AbstractIconLib.extend({
      mapping: {
